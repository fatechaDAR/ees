...
<div class="me-wrapper">
    ...
    <div class="me-top-bar">
        ...
        <div class="me-filters">
            <form method="GET" action="{{ route('monitoring.index') }}" style="display: flex; gap: 12px; align-items: center;">
                ...
                <select name="division_id" class="filter-select" onchange="this.form.submit()">
                    <option value="">Semua Divisi</option>
                    @foreach($divisions as $divisi)
                        <option value="{{ $divisi->id }}" {{ request('division_id') == $divisi->id ? 'selected' : '' }}>{{ $divisi->name }}</option>
                    @endforeach
                </select>
                <button type="button" class="btn-icon-filter" title="Filter Lanjutan">
